...
sort -t '|' -k3 -f "$CONFIG_FILE" | while IFS='|' read -r filename url title; do
    ...
    if [[ -f "$FILE_PATH" ]]; then
        TARGET_LINE=$(head -n 30 "$FILE_PATH" | grep -m 1 -iE '^[!#].*(modified|updated|date|version)' | tr -d '\r')
        
        if [[ -n "$TARGET_LINE" ]]; then
            if DATE_MATCH=$(echo "$TARGET_LINE" | grep -oE '20[0-9]{2}[-/.][0-9]{2}[-/.][0-9]{2}' | head -n 1); then
                LAST_UPDATE=$(echo "$DATE_MATCH" | tr './' '-')
            elif DATE_MATCH=$(echo "$TARGET_LINE" | grep -oE '20[0-9]{2}(0[1-9]|1[0-2])(0[1-9]|[12][0-9]|3[01])' | head -n 1); then
                Y=$(echo "$DATE_MATCH" | cut -c1-4)
                M=$(echo "$DATE_MATCH" | cut -c5-6)
                D=$(echo "$DATE_MATCH" | cut -c7-8)
                LAST_UPDATE="$Y-$M-$D"
            elif DATE_MATCH=$(echo "$TARGET_LINE" | grep -oE '[0-9]{2}[-/.][0-9]{2}[-/.]20[0-9]{2}' | head -n 1); then
                D=$(echo "$DATE_MATCH" | cut -c1-2)
                M=$(echo "$DATE_MATCH" | cut -c4-5)
                Y=$(echo "$DATE_MATCH" | cut -c7-10)
                LAST_UPDATE="$Y-$M-$D"
            elif echo "$TARGET_LINE" | grep -iqE '(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)'; then
                CLEAN_STR=$(echo "$TARGET_LINE" | sed -E 's/^[!#].*(modified|updated|date|version)[^a-zA-Z0-9]*//i' | sed -E 's/ at / /gi' | tr -d '()[],')
                PARSED=$(date -u -d "$CLEAN_STR" +'%Y-%m-%d' 2>/dev/null)
                if [[ -n "$PARSED" ]]; then
                    LAST_UPDATE="$PARSED"
                fi
            fi
